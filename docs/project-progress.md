...
```text
权限与复审流程已完成，正在补齐用户显示名等演示细节，并进入最终验收与课程报告材料整理
```

最新补充：

- 已将登录账号和页面显示用户名分离。
- `user` 表新增 `display_name` 字段，用于顶部栏、用户管理列表和登录成功提示。
- `admin`、`reviewer`、`user` 三个演示账号分别显示为“系统管理员”“审核员一号”“普通用户一号”，不再把角色或登录账号当作页面用户名。
- 注册时可填写“用户名”（展示名），后端默认新用户角色仍为“一般用户”。
